...
- **資料來源（原本文件寫錯，已更正）**：`extension/popup.js` 沒有任何程式碼「特意」處理 `_sign`，但簽名會其實跟一般全握場次共用同一支 `ticket-api.fortunemeets.app/user/history2` JSON API，簽名會項目的 `item.prizeId` 本身就長得像 `"0801_sign"`（日期+`_sign`，不是純數字）——`parseFullApiResults()` 組 `order_id` 時把 `item.prizeId` 原封不動嵌進字串（`` `full:${group}_${singleNum}${suffix}:${item.prizeId}:${memberName}:${lotteryRound}` ``），意外讓最終的 `order_id` 剛好含有 `_sign`，後端 `strings.Contains(r.OrderID, "_sign")` 因此正確分流到 `SignEvent`。這代表**簽名會資料現在就是靠既有全握抓取流程「順便」正確處理掉的，不是沒有資料來源**，`event_date`/`member_name`/`applied_count`/`won_count`（`item.resultInfo.win`）/`lottery_round` 都跟全握走同一套解析邏輯，理論上都是對的
- `Venue`（場地）欄位：因為走同一套 `parseFullApiResults`，`prizeInfo.date`（例：`2026年8月1日（土）＠京都パルスプラザ`）本來就會被解析出 `venue`，新版簽名會資料的 `venue` 是有值的；但舊版（較早的資料）`prizeInfo.date` 不帶場地文字，解析出來是空字串。所以 `PushFullRecords` 建立/更新 `SignEvent` 時，`venue` 優先吃 client 端算好的 `r.Venue`，空字串才退回跟全握同一套 `venueMap`（`group`+`single_number`+`event_date`）反推（簽名會場次掛在同一張單曲同一天的活動上，理論上跟全握共用同一個場地）（見 #117/#118）
...
4. **`/admin/venues` 場地管理**（AdminVenuesView.vue，內部用 el-collapse 分三塊，架構完全比照單曲名稱頁） — 缺少場地：問題列表（`GetVenueIssues`，全握実体場次**+簽名會**（見 #119）venue 空白的 (group, single_number, event_date) 組合，兩邊同一個組合會合併成一列不會重複列出）+ 可編輯場地 + 提交（`FixVenue` 同時回填全握跟簽名會既有空白紀錄，並寫入 `venues` 表供未來自動套用）；場地批次登記：貼上多行 `group,single_number,event_date,venue` 一次送出（`BulkSetVenues`，同樣兩邊都回填），前端排重用 group+single_number+event_date；已知場地：依 group 篩選瀏覽（`GetKnownVenues`，合併 `venues` 表登記 + full_records 已有場地文字的實際資料並顯示來源，**目前還沒把簽名會的場地文字併進來源推測**，見 #119），可編輯
...
116. 「各次應募中選率比較」折線圖（`RecordsAnalysisView.vue`「個握分析」+ `DashboardView.vue`「全員統計」個握總表，#89 引入的同一套邏輯，兩處各自實作一份）只選「團體」下拉、不選「成員」時，圖表還是把三個團體所有成員的曲線都畫出來，沒有照選的團體篩選。根因：`applyChartFilter()`/`rApplyChartFilter()`（決定 legend 要選取誰）跟 `memberMap`/`rMemberMap` 初始化那段，判斷「要不要篩選」只看 `chartFilterMembers`/`rChartFilterMembers`（成員下拉）是否有值，`chartFilterGroup`/`rChartFilterGroup`（團體下拉）原本的設計只用來縮小「成員」下拉本身的選項範圍，不會直接影響圖表——只選團體時 `hasFilter` 判斷是 false，等於沒篩，圖表照樣畫全部團體。
    - 修正：新增 `chartAllowedNames()`/`rChartAllowedNames()`：成員有選就用成員選的（最精確）；沒選成員但有選團體，退回用「該團體全部成員」當範圍（`chartMemberOptions`/`rChartMemberOptions` 本來就是團體篩過的選項，直接拿來用）；兩個都沒選才是真的全部顯示
    - 純前端邏輯調整，Vite dev server HMR 直接生效，用瀏覽器實際點選「乃木坂46」驗證過，legend 只會選取乃木坂46的成員
117. 簽名會紀錄新增「場地」欄位。`SignEvent` model 原本沒有 `Venue` 欄位，即使共用的 `FullPayload`（`POST /scrape/full/push`）payload 早就有 `venue`，`full_scraper.go` 建立 `SignEvent` 時完全沒接住，單純是漏接。
    - `models/sign_event.go` 加 `Venue` 欄位（`AutoMigrate` 既有清單已經包含 `SignEvent`，重啟後端會自動補欄位，不用手動 migration）
    - `PushFullRecords` 建立新的 `SignEvent`／更新既有 `SignEvent`（`applied_count`/`won_count` 有變動時）都優先吃 client 端算好的 `r.Venue`，空字串才退回查 `venueMap`（`group`+`single_number`+`event_date`）反推——見下方 #118，簽名會其實有自己的場地來源（新版資料才有，舊版沒有）
    - `GetAdminSignEvents`（`admin.go`）的 `SignEventRow` 手動 select 列表加 `Venue` 欄位（`Select("sign_events.*, ...")` 本來就會帶到新欄位，只是輸出用的 struct 沒宣告會被 GORM Scan 忽略）；`GetSignEvents`/`PreviewUserSignEvents` 直接吃整個 `models.SignEvent`，不用另外改
    - `AdminSignEventsView.vue`、`FullSignEventsView.vue`、`AdminMaintenanceView.vue`（刪除資料預覽的簽名會分頁）三個前端表格都加上「場地」欄（`min-width:160`，比照 `FullView.vue` 場地欄的慣例）
    - 用瀏覽器實際點開 `/admin/sign-events` 驗證過欄位正常顯示；當下資料庫裡的 7 筆簽名會紀錄一開始誤判成「外部手動寫入的測試資料」，後來查 `order_id`/`scraped_at` 並跟使用者確認過，其實是正常全握同步時 `ticket-api.fortunemeets.app` 回傳的真實資料，見上方「簽名會」章節、#118
118. #117 上線後追查「這批簽名會資料到底怎麼進來的」，發現一開始的判斷是錯的（見上方「簽名會」章節開頭已更正的說明）：`extension/popup.js` 沒有專門處理 `_sign` 的程式碼，但簽名會項目的 `item.prizeId` 本身就是 `"日期_sign"` 格式（例：`"0801_sign"`），`parseFullApiResults()` 組 `order_id` 時把 `prizeId` 原封不動嵌進去，意外讓 `order_id` 含有 `_sign`，後端的字串比對因此「順便」正確分流——簽名會其實一直都跟全握共用同一套抓取跟解析邏輯，只是沒人設計成這樣，是巧合。
    - 因為走同一套 `parseFullApiResults`，`event_date`/`member_name`/`applied_count`/`won_count`（`item.resultInfo.win`）/`lottery_round`，以及**場地**（`prizeInfo.date` 裡 `＠` 後面的文字）理論上都已經正確解析——但使用者確認：只有**新版**簽名會資料的 `prizeInfo.date` 有帶場地文字，**舊版**沒有帶，這種情況下 `r.Venue` 會是空字串
    - 修正 #117 遺留的一個不一致：`PushFullRecords` 新建 `SignEvent` 時本來就有做「`r.Venue` 優先、`venueMap` 反推當備援」，但**既有列的更新路徑**只查了 `venueMap`，沒有優先看 `r.Venue`，兩處優先順序不一致；改成兩處都是同一套邏輯，這樣新版資料重新同步時會直接吃到自己的場地文字，舊版資料才會退回用全握 `venueMap` 反推
    - 提醒：`parseFullApiResults(results, singleNum, group)` 的 `singleNum` 是外層目前掃描到第幾張單曲頁面帶進來的參數，不是從 `prizeInfo.event` 文字（例：「...17thシングル「Kind of love」...」）解析出來的；已知的這幾筆資料剛好對得上，但如果簽名會項目有可能在掃描「別的單曲頁面」時被回傳，現在的邏輯會存錯 `single_number`，這點還沒有處理，是已知風險，之後如果發現簽名會單曲號對不上再回來修
119. #118 提到簽名會（`SignEvent`）舊版資料沒有場地文字、要靠 `venueMap` 反推，但反推能不能成功取決於同一個 (group, single_number, event_date) 有沒有登記過場地——原本「有哪些場次缺場地」這件事只有 `GetVenueIssues` 掃 `full_records`（実体場次）看得到，簽名會缺場地完全不會被列出來，管理者無從得知、也無從登記。
    - `GetVenueIssues` 新增掃 `sign_events` 場地空白的 (group, single_number, event_date) 組合（簽名會沒有 `event_type` 欄位，本身一定是実体活動，不用像全握那樣篩 `event_type = '実体'`）；如果全握那邊已經列過同一個組合，直接把簽名會的筆數併進同一列的 `Count`，不會讓管理者看到同一個場次重複出現兩列
    - `FixVenue`（單筆修正）／`BulkSetVenues`（批次登記）登記場地時，除了原本回填 `full_records`，一併回填 `sign_events` 裡符合 (group, single_number, event_date) 且場地為空或跟登記值不同的紀錄——場地本來就是掛在同一個場次上，兩張表理論上該是同一個值
    - 直接查資料庫驗證過：目前 `full_records` 場地空白 0 組，`sign_events` 場地空白 6 組（乃木坂 34/35/36/38/39th 各一筆、日向坂 9th 一筆），這 6 組原本完全不會出現在問題列表，這次修完會正確列出來
    - **已知缺口**：`GetKnownVenues`（已知場地瀏覽）目前還沒把簽名會的場地文字併進「來源推測」，只有 `venues` 表登記 + `full_records` 實際資料兩種來源，之後如果需要瀏覽簽名會場地來源可以再補
120. 排查簽名會單曲名稱時發現全握（`FullRecord`）跟簽名會（`SignEvent`）共用的 `PushFullRecords`（`full_scraper.go`）**整支程式從來沒有查過 `titles` 表**——`single_name` 是擴充功能端 `parseFullApiResults()` 只用「目前掃描到第幾張單曲頁」湊出的空白模板（例："17thシングル"），跟每一筆 `item.prizeInfo.event` 裡實際寫的活動描述文字完全無關，那段文字目前只拿來判斷 `event_type`（含不含「リアル」）。個握/花費（`PushRecords`/`PushPurchases`）在 #109/#111 修過的「一律查表套用」邏輯，從來沒有套用到全握這邊，就算 `titles` 表已經登記了正確標題也不會生效。
    - 修正：`PushFullRecords` 新增 `resolveSingleName(group, singleNumber, fallback)`，比照 #109/#111 的做法（不透過任何「看起來像不像正確」的啟發式判斷，只要 `titleMaps.Singles[titleKey{Group, SingleNumber}]` 有登記就套用，沒登記過就用原本的 fallback），套用在全握跟簽名會的新建/既有列更新兩條路徑，總共四處
    - 既有列的 `single_name` 回填條件原本是 `existing.SingleName == "" && r.SingleName != ""`（只補空白），改成 `existing.SingleName != correctedName`（只要跟查表結果不一樣就更新）——原本的條件永遠不會把「17thシングル」這種非空但沒有標題的舊格式換成有標題的版本，因為它不是空字串
