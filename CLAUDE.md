...
│   │   └── title_correction.go   # タイトル未定 修正對照表（(group, single_number) 複合唯一鍵 → single_name，供自動套用）
...
- 解決方式：透過 `/admin` 頁面的 `PUT /api/admin/title` 手動修正，或 `POST /api/admin/titles/bulk` 批次登記
- `FixSingleTitle` 會同時更新所有使用者的個握（records）與購入（purchases）資料
- 修正結果同時寫入 `title_corrections` 資料表，供後續新抓取自動套用建議名稱

**Group 安全性**：`title_corrections` 唯一鍵是 `(group, single_number)` 複合鍵，不是單純 `single_number`——三個團體都有自己的「第 N 張單曲」編號，若只用 single_number 當鍵，不同團體同號的單曲會互相覆蓋。對應地：
- `loadCorrectionMap()` 回傳 `map[titleCorrectionKey]string`（`titleCorrectionKey{Group, SingleNumber}`），所有寫入路徑（PushRecords、UpdateTitles、PushPurchases）查表時都要帶 group
...
- 既有舊資料（修正這個 bug 之前建立的 `title_corrections`）group 一律是空字串，不會跟任何真實 group 撞鍵，等同失效，需要透過 `/admin` 重新登記
...
3. **タイトル未定 修正** — 問題列表（含團體欄位）+ 可編輯建議標題 + 提交（`FixSingleTitle` 同時更新 records + purchases，並寫入 title_corrections）；下方另有批次登記區塊，貼上多行 `group,single_number,single_name` 一次送出（`BulkSetTitles`），不需要先出現問題才能登記
